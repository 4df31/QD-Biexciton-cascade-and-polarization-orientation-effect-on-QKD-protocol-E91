...
def generate_plots(df: pd.DataFrame):
    ...
    fig1.update_traces(
        marker=dict(size=12, opacity=0.8, line=dict(width=1.5, color='DarkSlateGrey'))
    )
    fig1.update_layout(
    ...
    # =========================================================
    # PLOT 2: Mean with Error Bars (Std Dev)
    # =========================================================
    # Group by states to calculate mean and standard deviation of R2_Score
    df_stats = df.groupby("States")["R2_Score"].agg(["mean", "std"]).reset_index()
    df_stats["std"] = df_stats["std"].fillna(0) # Handle cases with single data points
    ...
    fig2.update_traces(
        marker=dict(size=12, opacity=1.0, color='royalblue', line=dict(width=1.5, color='DarkSlateGrey')),
        error_y=dict(thickness=2, width=6, color='DarkSlateGrey')
    )
    fig2.update_layout(
        font=dict(family="Arial, sans-serif", size=14),
        xaxis=dict(showgrid=True, gridwidth=1, gridcolor='LightGray'),
    ...
    # ---------------------------------------------------------
    # DISPLAY COMMAND (Strictly Plot 1 only)
    # ---------------------------------------------------------
    fig1.show()
