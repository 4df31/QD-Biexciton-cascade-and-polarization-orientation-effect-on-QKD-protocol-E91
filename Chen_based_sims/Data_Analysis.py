...
def process_simulation_files(directory: str = "."):
    ...
    pattern = r"gpu_alpha_([\d\.]+)pi_Pt_([\d\.]+)_(\d+)x(\d+)_mesh_([\d\.E\+\-]+)_states"
    
    # We look for all csv files starting with 'gpu_alpha'
    search_path = os.path.join(directory, "gpu_alpha_*.csv")
    file_list = glob.glob(search_path)
    
    if not file_list:
        print(f"No files matching 'gpu_alpha_*.csv' found in {os.path.abspath(directory)}")
        return pd.DataFrame()
...
def generate_scatter_plot(df: pd.DataFrame):
    ...
    fig = px.scatter(
        ...
        log_x=True,  # Logarithmic scale makes states like 1e3 to 4.5e4 much easier to read
